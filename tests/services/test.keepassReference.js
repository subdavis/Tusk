...
describe('Keepass References', function () {
  ...
  describe('Current Entry', function () {
    ...
    it('should support a custom field name', function () {
      refService
        .resolveReference('{S:EmailAddress}', entry, entries)
        .should.equal(entry.emailAddress);
    });
  });
  ...
  describe('All entries', function () {
    ...
    it('should search in title', function () {
      refService
        .resolveReference('{REF:I@T:' + entry2.title + '}', entry, entries)
        .should.equal(entry2.id);
    });
    it('should search in user name', function () {
      refService
        .resolveReference('{REF:I@U:' + entry2.userName + '}', entry, entries)
        .should.equal(entry2.id);
    });
    it('should search in password', function () {
      refService
        .resolveReference('{REF:I@P:' + entry2.password + '}', entry, entries)
        .should.equal(entry2.id);
    });
    it('should search in url', function () {
      refService
        .resolveReference('{REF:I@A:' + entry2.url + '}', entry, entries)
        .should.equal(entry2.id);
    });
    it('should search in notes', function () {
      refService
        .resolveReference('{REF:I@N:' + entry2.notes + '}', entry, entries)
        .should.equal(entry2.id);
    });
    it('should search in id', function () {
      refService
        .resolveReference('{REF:I@I:' + entry2.id + '}', entry, entries)
        .should.equal(entry2.id);
    });
    ...
    it('should search in custom strings', function () {
      refService
        .resolveReference('{REF:I@O:' + entry2.emailAddress + '}', entry, entries)
        .should.equal(entry2.id);
    });
  });
  ...
  describe('interpolating multiple references', function () {
    ...
    it('should work with a reference at the start', function () {
      refService
        .processAllReferences(3, '{TITLE} ', entry, entries)
    ...
    it('should work with multiple references', function () {
      refService
        .processAllReferences(3, ' {TITLE} {TITLE} ', entry, entries)
        .should.equal(' ' + entry.title + ' ' + entry.title + ' ');
    });
    ...
    it('should return unrecognized expressions as-is', function () {
      refService
        .processAllReferences(3, ' {TITLE} {nothing} ', entry, entries)
        .should.equal(' ' + entry.title + ' {nothing} ');
    });
  });
});
