/*
	Offloader: safeguard against physically compromised machines by
	offloading the keys used to protect data in memory to some remote
	third party
*/
...
function Offloader(settings, lambda) {
	...
	my.encrypt = (keyname, valueJson, expireTime) => {
		...
		let seconds = Math.floor(expireTime / 1000)
		return Promise.all([hashPromise, tokenPromise]).then(values_a => {
			...
			return utils.encrypt(valueString).then(values_b => {
				...
				return lambda.save(hash, JSON.stringify(dataForLambda), seconds, token).then(() => {
					return encB64;
				});
			});
		});
	...
	my.decrypt = (keyname, encB64) => {
		...
		return Promise.all([hashPromise, tokenPromise]).then(values_a => {
			...
			return lambda.get(hash, token).then(aesDataString => {
				return utils.decrypt(aesDataString, encB64).then(valueString => {
